...
aaaa();
```
외부함수 aaaa에 내부함수 bbbb가 정의되어 있음  
내부함수는 외부함수의 지역변수에 접근 가능하므로, 외부함수 aaaa를 호출하고 내부함수 bbbb가 호출되었을 때
결과는 red가 나오게 됨  
외부함수의 실행시간이 끝나고 내부함수는 외부함수의 변수를 사용할 수 있음
